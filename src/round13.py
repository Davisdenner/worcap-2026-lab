"""Replace only S10 tropical linear head by a fixed small RBF search."""
...
ART=ROOT/'data/processed/round13'
OUT=REPORT/'round13'
PROTOCOL=ROOT/'experiments/ROUND13.md'
DEV=r11.DEV
CONFIGS={f'rbf_h{h:g}_a{a:g}':(h,a) for h in (.5,2.) for a in (.3,3.)}


def locked():
    ART.mkdir(parents=True,exist_ok=True); OUT.mkdir(parents=True,exist_ok=True)
    value=dict(protocol_sha256=r9.digest(PROTOCOL),script_sha256=r9.digest(ROOT/'src/round13.py'),
               ...
               configurations=CONFIGS,official_only=True,reference='S10')
...
def fit_kernel(z,y,h,alpha):
    z=np.asarray(z,dtype=float); y=np.asarray(y,dtype=float)
    if len(z)!=len(y) or len(z)<2 or h<=0 or alpha<=0 or not np.isfinite(z).all() or not np.isfinite(y).all():
        raise ValueError('Invalid kernel training matrices or parameters')
    median=float(np.median(pdist(z,'sqeuclidean')))
    if median<=0: raise ValueError('Degenerate kernel distances')
    bandwidth=h*median
    kernel=np.exp(-cdist(z,z,'sqeuclidean')/bandwidth)
    factor=cho_factor(kernel+alpha*np.eye(len(z)),lower=True)
    inverse_ones=cho_solve(factor,np.ones(len(z)))
    intercept=inverse_ones@y/inverse_ones.sum()
    return dict(training_scores=z.copy(),bandwidth=bandwidth,alpha=alpha,factor=factor,intercept=intercept,
                median_training_distance=median,bandwidth_factor=h)


def predict_kernel(model,z,y):
    """Dual prediction, retaining the training targets as part of the fitted model."""
    kernel=np.exp(-cdist(np.asarray(z,dtype=float),model['training_scores'],'sqeuclidean')/model['bandwidth'])
    weights=cho_solve(model['factor'],kernel.T).T
    return weights@y+(1-weights.sum(axis=1))[:,None]*model['intercept']
...
def inputs(cutoff):
    ...
    linear=joblib.load(r11.ART/f'{cutoff}_fine32.joblib')
    if linear['training_target_end']!=f'{cutoff-1}-12' or not linear['official_only']:
        raise ValueError('Wrong PLS cutoff')
    x=r12.features(cutoff,idx); xv=r12.features(cutoff,origins)
    z=linear['pls'].transform(x/linear['x_scale'])/linear['score_scale']
    zv=linear['pls'].transform(xv/linear['x_scale'])/linear['score_scale']
    base=linear['climatology'][(origins+1)%12]
    control=np.maximum(base+r11.anomaly_prediction(linear,xv),0).reshape(24,121,261)
    ...
    y=tp[idx+1,180:].reshape(len(idx),-1)-linear['climatology'][(idx+1)%12]
    target_path=ART/f'{cutoff}_training_targets.npy'
    if target_path.exists(): np.testing.assert_array_equal(y,np.load(target_path))
    else: np.save(target_path,y)
    save_json(ART/f'{cutoff}_training.json',dict(training_pairs=len(idx),last_target=f'{cutoff-1}-12',
              first_forecast=f'{cutoff}-01',linear_control_reproduced=True,official_only=True,
              training_targets_sha256=r9.digest(target_path),
              pls_model_sha256=r9.digest(r11.ART/f'{cutoff}_fine32.joblib')))
    return z,zv,y,base,ref


def candidates(cutoff,names=None):
    locked(); z,zv,y,base,ref=inputs(cutoff); result={}
    for name in (CONFIGS if names is None else names):
        h,a=CONFIGS[name]; path=ART/f'{cutoff}_{name}.joblib'
        if not path.exists():
            model=fit_kernel(z,y,h,a)
            model.update(cutoff=cutoff,training_targets_sha256=r9.digest(ART/f'{cutoff}_training_targets.npy'),official_only=True)
            joblib.dump(model,path)
        model=joblib.load(path)
        if model['cutoff']!=cutoff or model['training_targets_sha256']!=r9.digest(ART/f'{cutoff}_training_targets.npy'):
            raise ValueError('Stale kernel model')
        anomaly=predict_kernel(model,zv,y)
        np.testing.assert_array_equal(anomaly,predict_kernel(joblib.load(path),zv,y))
        ...
        if not np.isfinite(pred).all() or (pred<0).any(): raise ValueError('Invalid predictions')
        output=ART/f'{cutoff}_{name}.npy'
        if output.exists(): np.testing.assert_array_equal(pred,np.load(output))
        else: np.save(output,pred)
        result[name]=pred
        print('KERNEL',cutoff,name,'pairs',len(y),flush=True)
    return result
...
def final():
    ...
    metadata.update(experiment='round13',model=name,base_model='S10',official_data_only=True,external_sources=[],
...
def summarize():
    ...
    lines=['# Rodada 13 — componente tropical não linear','',
